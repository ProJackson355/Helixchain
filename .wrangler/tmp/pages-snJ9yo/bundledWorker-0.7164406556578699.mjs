...
var worker_default = {
  async fetch(request, env) {
    ...
    let targets;
    try {
      targets = nodeBases(env.HELIX_NODE_URL).map((base) => upstreamUrl(base, path, incoming.search));
    ...
    const isMiningStatus = path.startsWith("/mine/status/");
    const canFailOver = method === "GET" && !isMiningStatus || method === "HEAD" || method === "POST" && (path === "/transaction" || /^\/transaction\/[0-9a-f]{64}\/cancel$/.test(path));
...
//# sourceMappingURL=bundledWorker-0.7164406556578699.mjs.map
