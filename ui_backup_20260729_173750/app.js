...
async function signPayload(privateKey, payload) {
  ...
  if (
    payload && payload.sender && payload.receiver
    && Object.prototype.hasOwnProperty.call(payload, 'amount')
    && !Object.prototype.hasOwnProperty.call(payload, 'chain_id')
  ) await applyTransactionEnvelope(payload);
  const secp256k1 = await getSecp256k1();
...
async function applyTransactionEnvelope(payload, sequenceOverride = null) {
  const info = await api('GET', `/transaction/envelope/${payload.sender}`);
  payload.chain_id = info.chain_id;
  payload.sequence = sequenceOverride === null
    ? Number(info.next_sequence) : Number(sequenceOverride);
  payload.valid_until_height = Number(info.valid_until_height);
  return payload;
}
...
function showPanel(name) {
  ...
  if (name === 'leaderboard') loadLeaderboard(LEADERBOARD_PAGE);
...
function transactionAmountText(tx, sign = '') {
  ...
  if (type === 'cancel') return 'Sequence cancellation';
...
const TOKEN_CHART_MIN_WIDTH = 280;
const TOKEN_CHART_MAX_WIDTH = 1800;
let TOKEN_CHART_HEIGHT = 320;
let TOKEN_CHART_WIDTH = Math.min(
  720,
  Math.max(TOKEN_CHART_MIN_WIDTH, Math.floor((window.innerWidth - 64) / 20) * 20),
);
...
function renderTokenPriceChart(container, token, rawPoints) {
  ...
  const isWallet = !!token.__wallet;
  const allPoints = rawPoints.map(point => ({
    ...point,
    price: isMint ? Number(point.supply)
      : isNft ? Number(point.price)
      : isWallet ? Number(point.worth_hlx)
      : marketPointPrice(point, token.decimals),
  }))
  ...
  if (!allPoints.length) {
    const emptyMessage = isMint
      ...
      : isWallet ? 'Wallet worth history appears after the first confirmed block.'
  ...
  const headTitle = isMint ? 'HLX minted over time'
    : isNft ? 'Confirmed NFT sale history'
    : isWallet ? 'Confirmed wallet worth history'
  ...
  container.innerHTML = `<div class="price-chart-head"><strong>${escapeHtml(headTitle)}</strong>
    ...
    <div class="chart-scroll"><div class="chart-viewport" style="height:${TOKEN_CHART_HEIGHT}px;width:${TOKEN_CHART_WIDTH}px">
      <svg class="price-chart-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="${isMint ? 'HLX minted over time' : isNft ? escapeHtml(token.name) + ' confirmed sale price in HLX over time' : isWallet ? 'Confirmed wallet worth in HLX over time' : escapeHtml(token.symbol) + ' price in HLX over time'}">
    ...
    <div class="chart-help token-address"><span>${realUpdates} ${isMint || isWallet ? `block${realUpdates === 1 ? '' : 's'}` : isNft ? `confirmed sale${realUpdates === 1 ? '' : 's'}` : `confirmed market update${realUpdates === 1 ? '' : 's'}`} in view</span><span>Drag to pan &middot; scroll to zoom &middot; double-click to reset</span></div>`;
...
function secondaryChartContainer(target) {
  return target.closest?.('#nft-price-chart, #leaderboard-worth-chart') || null;
}

// NFT and leaderboard charts live outside token-market-detail, so their
// dynamically rendered controls need document-level delegated handlers.
document.addEventListener('click', event => {
  const container = secondaryChartContainer(event.target);
  if (!container || !TOKEN_CHART_TOKEN) return;
  if (event.target.closest('[data-chart-fit]')) {
    if (TOKEN_CHART_INTERVAL) {
      const { earliest, latest } = tokenChartBounds(TOKEN_CHART_POINTS);
...
document.addEventListener('input', event => {
  const container = secondaryChartContainer(event.target);
  if (!container || !event.target.matches('#token-chart-height, #token-chart-width')) return;
  const value = Number(event.target.value);
  if (!Number.isFinite(value)) return;
  const viewport = container.querySelector('.chart-viewport');
  if (event.target.matches('#token-chart-height')) {
  ...
  if (event.target.nextElementSibling) event.target.nextElementSibling.textContent = `${value}px`;
});
  ...
  try {
    ...
    el.innerHTML = `<div class="tx-list">${txs.map(tx => {
      ...
      const label = type === 'token_create' ? `Create ${tx.symbol || 'token'}`
        ...
        : type === 'nft_buy' ? 'Buy NFT'
        : type === 'cancel' ? 'Cancel pending transaction' : 'Pending';
...
document.addEventListener('click', async event => {
  ...
  try {
    const publicKeyPem = await exportPublicKeyPEM(S.publicKey);
    const detail = await api('GET', `/transaction/${txId}`);
    const original = detail.transaction;
    let result;
    if (original && original.sequence !== undefined && original.sequence !== null) {
      const payload = {
        tx_type: 'cancel', sender: S.address, receiver: S.address, amount: 0,
        fee: Math.max(currentTransactionFee(), Number(original.fee || 0) + 1),
      };
      await applyTransactionEnvelope(payload, Number(original.sequence));
      payload.signature = await signPayload(S.privateKey, payload);
      payload.public_key = publicKeyPem;
      result = await api('POST', '/transaction', payload);
      if (!result.accepted) throw new Error(result.message || 'Cancellation replacement was rejected.');
    } else {
      const auth = { action: 'cancel_pending', sender: S.address, tx_id: txId };
      const signature = await signPayload(S.privateKey, auth);
      result = await api('POST', `/transaction/${txId}/cancel`, {
        sender: S.address, signature, public_key: publicKeyPem,
      });
      if (!result.cancelled) throw new Error(result.message || 'Cancellation was rejected.');
    }
    toast('Cancellation transaction submitted', 'ok');
    await loadPending();
    loadDashboard();
  } catch (error) {
  ...
  try {
    ...
    el.innerHTML = `<div class="tx-list">${txs.map(tx => {
      ...
      const label = type === 'token_create' ? `Created ${tx.symbol || 'token'}`
                  ...
                  : type === 'cancel' ? 'Transaction cancelled'
...
function renderActivityTransactions(result) {
  ...
  list.innerHTML = `<div class="tx-list">${transactions.map(tx => {
    ...
    const label = type === 'token_create' ? `Created ${tx.symbol || 'token'}`
      ...
      : type === 'cancel' ? 'Cancelled pending transaction'
...
// ============================================================
// SECTION 13b — Public wallet-worth leaderboard
// ============================================================
const LEADERBOARD_PAGE_SIZE = 25;
let LEADERBOARD_PAGE = 1;

function renderLeaderboardPagination(totalEntries) {
  const pagination = document.getElementById('leaderboard-pagination');
  const pages = Math.max(1, Math.ceil(Number(totalEntries || 0) / LEADERBOARD_PAGE_SIZE));
  if (!totalEntries) { pagination.innerHTML = ''; return; }
  const buttons = activityPageNumbers(LEADERBOARD_PAGE, pages).map(page => page === null
    ? '<button class="page-btn" type="button" disabled aria-hidden="true">&hellip;</button>'
    : `<button class="page-btn${page === LEADERBOARD_PAGE ? ' active' : ''}" type="button" data-leaderboard-page="${page}"${page === LEADERBOARD_PAGE ? ' aria-current="page"' : ''}>${page}</button>`
  ).join('');
  pagination.innerHTML = `<button class="page-btn" type="button" data-leaderboard-page="${Math.max(1, LEADERBOARD_PAGE - 1)}"${LEADERBOARD_PAGE <= 1 ? ' disabled' : ''}>&lsaquo;</button>${buttons}<button class="page-btn" type="button" data-leaderboard-page="${Math.min(pages, LEADERBOARD_PAGE + 1)}"${LEADERBOARD_PAGE >= pages ? ' disabled' : ''}>&rsaquo;</button>`;
}

async function loadLeaderboard(page = 1) {
  LEADERBOARD_PAGE = Math.max(1, Number(page) || 1);
  const list = document.getElementById('leaderboard-list');
  if (!list) return;
  list.innerHTML = '<div class="empty">Loading&hellip;</div>';
  try {
    const offset = (LEADERBOARD_PAGE - 1) * LEADERBOARD_PAGE_SIZE;
    const result = await api('GET', `/leaderboard?limit=${LEADERBOARD_PAGE_SIZE}&offset=${offset}`);
    const entries = result.entries || [];
    list.innerHTML = entries.length ? `<div class="tx-list">${entries.map(entry => {
      const isYou = S && entry.address === S.address;
      const excluded = Number(entry.unpriced_token_count || 0);
      return `<div class="tx-row tx-row-action" role="button" tabindex="0" data-leaderboard-address="${escapeHtml(entry.address)}">
        <div class="tx-icon ${entry.rank <= 3 ? 'sys' : 'in'}">#${escapeHtml(entry.rank)}</div>
        <div class="tx-body"><div class="tx-label">${isYou ? 'You · ' : ''}${escapeHtml(short(entry.address))}</div>
          <div class="tx-sub">${escapeHtml(entry.address)}</div>
          <div class="tx-sub">${escapeHtml(entry.hlx_balance)} HLX balance · ${escapeHtml(entry.estimated_token_value_hlx)} HLX token value${excluded ? ` · ${escapeHtml(excluded)} unpriced token${excluded === 1 ? '' : 's'} excluded` : ''}</div></div>
        <div class="tx-right"><div class="tx-amount in">${escapeHtml(formatWalletValue(Number(entry.estimated_total_hlx)))} HLX</div><div class="tx-meta">Estimated worth</div></div>
      </div>`;
    }).join('')}</div>` : '<div class="empty">No funded confirmed addresses yet.</div>';
    renderLeaderboardPagination(result.total || 0);
  } catch (error) {
    list.innerHTML = `<div class="empty">${escapeHtml(error.message || 'Could not load the leaderboard.')}</div>`;
    document.getElementById('leaderboard-pagination').innerHTML = '';
  }
}

async function openLeaderboardWallet(address) {
  const detail = document.getElementById('leaderboard-detail');
  const chart = document.getElementById('leaderboard-worth-chart');
  detail.hidden = false;
  document.getElementById('leaderboard-detail-address').textContent = address;
  document.getElementById('leaderboard-detail-summary').innerHTML = '';
  chart.innerHTML = '<div class="empty">Loading wallet-worth history&hellip;</div>';
  detail.scrollIntoView({ behavior: 'smooth', block: 'start' });
  try {
    const result = await api('GET', `/leaderboard/${address}/history?limit=2000`);
    const entry = result.entry || {};
    document.getElementById('leaderboard-detail-summary').innerHTML = [
      transactionDetailRow('Rank', entry.rank ? `#${entry.rank}` : 'Not ranked'),
      transactionDetailRow('Confirmed HLX', `${entry.hlx_balance ?? 0} HLX`),
      transactionDetailRow('Token spot value', `${entry.estimated_token_value_hlx ?? 0} HLX`),
      transactionDetailRow('Estimated total', `${entry.estimated_total_hlx ?? 0} HLX`),
    ].join('');
    renderTokenPriceChart(chart, {
      mint_address: `wallet:${address}`, symbol: 'Wallet', name: address,
      decimals: 0, __wallet: true,
    }, result.points || []);
  } catch (error) {
    chart.innerHTML = `<div class="empty">${escapeHtml(error.message || 'Could not load wallet history.')}</div>`;
  }
}

document.getElementById('btn-refresh-leaderboard')?.addEventListener('click', () => loadLeaderboard(LEADERBOARD_PAGE));
document.getElementById('btn-close-leaderboard-detail')?.addEventListener('click', () => {
  document.getElementById('leaderboard-detail').hidden = true;
});
document.getElementById('leaderboard-pagination')?.addEventListener('click', event => {
  const button = event.target.closest('[data-leaderboard-page]');
  if (button && !button.disabled) loadLeaderboard(Number(button.dataset.leaderboardPage));
});
document.getElementById('leaderboard-list')?.addEventListener('click', event => {
  const row = event.target.closest('[data-leaderboard-address]');
  if (row) openLeaderboardWallet(row.dataset.leaderboardAddress);
});
document.getElementById('leaderboard-list')?.addEventListener('keydown', event => {
  const row = event.target.closest('[data-leaderboard-address]');
  if (row && (event.key === 'Enter' || event.key === ' ')) {
    event.preventDefault();
    openLeaderboardWallet(row.dataset.leaderboardAddress);
  }
});
